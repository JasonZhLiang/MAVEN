...
public class NotificationClassesRepository {
	...
	public List<NotificationModel> findNotificationsByClassIds(List<Integer> classIds)
	{
		...
		Query query = entityManager.createQuery("SELECT DISTINCT nm " + 
				"FROM EntityNotificationModel nm, EntityNotificationClassesModel ncm " + 
				"WHERE nm.id = ncm.notificationid AND ncm.classid IN :classes " + 
				"ORDER BY nm.startdate, nm.expirydate DESC");
		query.setParameter("classes", classIds);
	...
	{
		Query query = entityManager.createQuery("SELECT COUNT(ncm) from EntityNotificationClassesModel ncm");
